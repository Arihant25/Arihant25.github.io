<script lang="ts">
	...
	// Optimized card-sized cover generated by optimize-assets.js
	const coverImagePath = `/research/papers/${paper.slug}/_opt/cover-card.webp`;
	...
	function handleMouseEnter() {
		if (!isMobile) {
			isHovered = true;
			// Warm up the detail page and its cover while the user hovers
			preloadData(`/research/${paper.slug}`).catch(() => {});
			const img = new Image();
			img.src = `/research/papers/${paper.slug}/_opt/cover.webp`;
		}
	}
...
>
	...
	<div class="poster-container relative aspect-[2/3] w-full overflow-hidden">
		{#if !coverLoaded}
			<div class="poster-skeleton absolute inset-0" aria-hidden="true"></div>
		{/if}
		<img
			src={coverImagePath}
			alt={paper.title}
