<script lang="ts">
	...
	// Optimized WebP variants generated by optimize-assets.js
	const coverImagePath = `/research/papers/${paper.slug}/_opt/cover.webp`;
	...
	onMount(async () => {
		...
		coverPreload.onload = () => (coverLoaded = true);
		coverPreload.onerror = () => (coverLoaded = true);
		coverPreload.src = coverImagePath;
		...
		pages.push(coverImagePath);

		for (let i = 1; i <= paper.pageCount; i++) {
			pages.push(`/research/papers/${paper.slug}/_opt/${paper.slug}-${i}.webp`);
		}
	...
	// Lazily initialize the audio player when it scrolls into view, so the
	// (large) audio file doesn't compete with the paper pages for bandwidth.
	let audioInitialized = false;
	$effect(() => {
		if (!browser || !waveformContainer || audioInitialized) return;

		const observer = new IntersectionObserver(
			(entries) => {
				if (entries[0].isIntersecting && !audioInitialized) {
...
<svelte:head>
	...
	<link rel="preload" as="image" href="/research/papers/{paper.slug}/_opt/cover.webp" />
...
>
	<div class="mx-auto max-w-7xl px-5 pt-20 pb-12 sm:px-6 lg:px-8">
		...
		<!-- Flipbook Container -->
		<div class="flipbook-area relative mb-12 flex justify-center">
			<!-- Book skeleton shown until the cover image is ready -->
			{#if !coverLoaded}
				<div class="book-skeleton" aria-hidden="true">
					<div class="book-skeleton-spine"></div>
				</div>
			{/if}

			<div
				bind:this={flipBookContainer}
				class="flipbook-container transition-opacity duration-500"
...
<style>
	...
	/* Reserve space for the book while it loads */
	.flipbook-area {
		min-height: min(733px, 133vw);
	}

	/* Book-shaped shimmering skeleton */
	.book-skeleton {
		position: absolute;
		top: 0;
		left: 50%;
		...
		background: linear-gradient(
			100deg,
			var(--card-bg) 40%,
			color-mix(in srgb, var(--card-bg) 70%, var(--text-secondary)) 50%,
			var(--card-bg) 60%
		);
		background-size: 200% 100%;
		animation: bookShimmer 1.4s ease-in-out infinite;
